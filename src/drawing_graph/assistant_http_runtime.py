"""Process-local HTTP runtime assembling driver, facade, and product service.
...
class AssistantHttpRuntime:
    ...
    def __init__(self, config: AssistantHttpConfig, driver: Any, facade: Any, service: Any):
        self.config = config
        self.driver = driver
        self.facade = facade
...
def create_assistant_http_runtime(
    ...
    try:
        ...
        _close_quietly(driver)
        raise
    return AssistantHttpRuntime(config=config, driver=driver, facade=facade, service=service)
...
__all__ = ("AssistantHttpRuntime", "create_assistant_http_runtime")
